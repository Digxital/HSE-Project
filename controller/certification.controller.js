...
exports.getUserCertifications = async (req, res) => {
  try {
    ...
    const user = await findUserInTenant(userId, req.user.tenantId);
    if (!user) {
...
exports.createCertification = async (req, res) => {
  try {
    ...
    if (req.file) {
      fileMetadata = extractFileMetadata(req.file);
    }
    ...
    if (fileMetadata) {
      certificationData.file = fileMetadata;
      certificationData.fileUrl = getFileUrl(fileMetadata.path);
    }
    ...
    // Delete uploaded file if certification creation fails
    if (req.file) {
...
exports.updateCertification = async (req, res) => {
  try {
    ...
    const user = await findUserInTenant(certification.userId, req.user.tenantId);
    if (!user) {
      return res.status(404).json({
        success: false,
        message: "Certification not found",
    ...
    // Handle file upload - delete old file if new one is provided
    if (req.file) {
      // Delete old file if it exists
      if (certification.file && certification.file.path) {
        deleteFile(certification.file.path);
      }
      // Add new file metadata
      const fileMetadata = extractFileMetadata(req.file);
      updateData.file = fileMetadata;
      updateData.fileUrl = getFileUrl(fileMetadata.path);
    }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        file: updatedCertification.file || null,
    ...
    // Delete uploaded file if certification update fails
    if (req.file) {
      deleteFile(req.file.path);
    }
    return res.status(500).json({
