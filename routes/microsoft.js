...
function getMicrosoftConfigError() {
  const missing = [];
  ...
  return `Microsoft configuration is incomplete. Missing: ${missing.join(', ')}`;
}

function formatMicrosoftError(error, fallbackMessage) {
  ...
  return {
    ...
    body: {
      ...
      details: detail,
      microsoftErrorCode: microsoftError?.code || error.response?.data?.error || null,
    },
  };
}
...
async function getMicrosoftAccessToken() {
  const configError = getMicrosoftConfigError();
  if (configError) {
  ...
  try {
    ...
    wrappedError.microsoft = error.response.data;
    throw wrappedError;
  }
}
  ...
  try {
    ...
    if (configError) {
      return res.status(503).json({
        error: 'Microsoft configuration error',
        details: configError,
      });
    }
    ...
    console.error('❌ Error getting Microsoft token:', error.response?.data || error.message);
    const formattedError = formatMicrosoftError(
      error,
      'Failed to get Microsoft token',
    );
    res.status(error.status || formattedError.status).json(
      error.microsoft
        ? {
            ...formattedError.body,
            microsoft: error.microsoft,
          }
        : formattedError.body,
    );
  }
