      ...
      >
        ...
        {!isSubmitted ? (
          <>
            ...
            <div className="flex justify-center gap-4 mb-8">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  {star <= rating ? (
                    <svg className="w-12 h-12 text-[#C2410C] fill-current" viewBox="0 0 24 24">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                    </svg>
                  ) : (
                    <svg className="w-12 h-12 text-[#C2410C] stroke-current" viewBox="0 0 24 24" fill="none" strokeWidth="1.5">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                    </svg>
                  )}
                </button>
                ...
                {categories.map((category) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => {}}
                      className="w-4 h-4 rounded cursor-pointer accent-[#C2410C]"
                    />
            ...
            {/* Feedback Textarea */}
            <div className="mb-6">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">Add a note</label>
              <textarea
                value={feedback}
                onChange={(e) => setFeedback(e.target.value)}
                placeholder="Add a note..."
                rows={4}
                className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#C2410C] focus:border-transparent resize-none"
              />
            </div>
          ...
          <>
            ...
            <button
              onClick={handleClose}
              className="w-full py-3 bg-[#C2410C] hover:bg-[#a83409] dark:bg-[#C2410C] dark:hover:bg-[#a83409] text-white rounded-lg font-medium transition-colors"
            >
              Done
            </button>
          </>
